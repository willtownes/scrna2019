...
For model comparison on negative control data, choose a single plate from one individual to ensure no batch effects.

```{r}
cm<-colData(sce0)
set.seed(205) #reproducibility
ssg<-sample.int(nrow(sce0),size=5000) #subset random genes
ssc<-cm$individual=="NA19098" & cm$replicate=="r3"
sce<-sce0[ssg,ssc]
...
system.time(res2<-bic_all(rc2,Xrc))
dd$covar<-FALSE
...
plt_func(subset(pd,dat=="umi_counts" & covar))
ggs("gof_bic_tung_umi_covar.pdf",w=3,h=2)
plt_func(subset(pd,dat=="read_counts" & covar))
ggs("gof_bic_tung_reads_covar.pdf",w=3,h=2)
...
plt_func2(subset(pd,dat=="umi_counts"))
ggs("gof_bic_tung_umi_both.pdf")
plt_func2(subset(pd,dat=="read_counts"))+scale_y_log10()
ggs("gof_bic_tung_reads_both.pdf")
```
