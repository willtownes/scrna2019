...
sce_d<-sce[gm$dev<=J,]
sce_h<-sce[gm$hvg<=J,]
Yd<-round(counts(sce_d))
Yh<-round(counts(sce_h))
```
...
ggplot(pd2,aes(x=dim1,y=dim2,colour=celltype)) +geom_point(size=.5) +facet_wrap(~dimreduce,scales="free") +guides(colour=guide_legend(override.aes=list(size=2)))
if(sp){ ggsave(fp(pth,"muraro_umap.pdf"),width=7,height=4) }
```
...
f<-function(d,res=0.8){
  ...
  data.frame(ari=ari(cl,d$celltype), sil=silfunc(cl,dist(factors)))
}
res<-dd %>% group_by(dimreduce,genefilter) %>% group_modify(~f(.x))
(res<-as.data.frame(res))
write.table(res,fp(bp,"results/muraro_clusteval.txt"),row.names=FALSE,quote=FALSE)
```

plot clustering results

```{r}
#res<-read.table(fp(bp,"results/muraro_clusteval.txt"),header=TRUE)
...
write.table(res2,fp(bp,"results/muraro_clusteval2.txt"),row.names=FALSE,quote=FALSE)
```
