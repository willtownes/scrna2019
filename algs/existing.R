...
umap<-function(Y,L=2,...){
  #run umap (package=uwot) visualization on data matrix Y
  #we assume observations=cols of Y, features=rows of Y
  #note set pca=50 to mimic tSNE on top 50 PCs
  #if pca=NULL (default), no PCA is done.
  #umap expects data with rows=observations
  res<-uwot::umap(t(Y),n_components=L,ret_model=FALSE,ret_nn=FALSE,...)
  colnames(res)<-paste0("dim",1:L)
  as.data.frame(res)
}